    ...
    public partial class BuildAutomator
    {
        ...
        private void DisplayXcodeSettings()
        {
            ...
            if (platforms.Exists(x => x.TargetPlatform == BuildTarget.iOS || x.TargetPlatform == BuildTarget.tvOS))
            {
                ...
                EditorUserBuildSettings.iOSXcodeBuildConfig = (XcodeBuildConfig)EditorGUILayout.EnumPopup(new GUIContent("Xcode Scheme", "Scheme with which the project will be run in Xcode."), EditorUserBuildSettings.iOSXcodeBuildConfig);
#endif
                GUI.enabled = true;
            }
        }
